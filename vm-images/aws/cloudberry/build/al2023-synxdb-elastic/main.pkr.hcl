...
source "amazon-ebs" "base-build-image" {
  ...
  ami_name = format("synx-elastic-packer-%s-%s-%s", var.vm_type, var.os_name, formatdate("YYYYMMDD-HHmmss", timestamp()))
  ami_description = "SynxDB Elastic - Amazon Linux 2023 (al2023) Base AMI built via Packer"
...
build {
  ...
  provisioner "shell" {
    script = "scripts/system_add_synxdb_elastic_dependencies.sh"
  }
