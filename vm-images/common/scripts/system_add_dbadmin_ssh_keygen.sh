...
# system_prepare_image_capture.sh removes the keys and the marker before
# capture, so an image chained from this one regenerates them on first boot.
...
for user in gpadmin cbadmin; do
  ...
  if [ ! -f "${ssh_dir}/id_ed25519" ]; then
    ...
    runuser -u "${user}" -- sh -c "ssh-keygen -y -f '${ssh_dir}/id_ed25519' > '${ssh_dir}/id_ed25519.pub'"
  fi
